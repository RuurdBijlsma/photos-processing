...
def setup_embedder():
    ...
    tests_folder = Path(__file__).parents[2]
...
@pytest.mark.parametrize("query, img_index", [
    ("A sunset over naples.", 2),  # Index 2 corresponds to sunset_img
    ("A cat sleeping in a bed.", 0),  # Index 0 corresponds to cat_img
    ("A cluster of raspberry pis in front of a laptop.", 1),  # Index 1 corresponds to cluster_img
])
def test_clip_embedder(setup_embedder, query, img_index):
