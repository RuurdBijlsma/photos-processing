...
async def generate_thumbnails(image_info: BaseImageInfo) -> None:
    folder = app_config.thumbnails_dir / image_info.hash
    if not os.path.exists(folder):
        os.makedirs(folder)
    input_file = app_config.images_dir / image_info.relative_path

    await ffmpeg_thumbnails(input_file, app_config.thumbnail_heights, folder)
    if image_info.relative_path.suffix in app_config.video_suffixes:
        await generate_web_video(input_file, app_config.web_video_height, folder)
