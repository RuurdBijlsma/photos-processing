...
from app.processing.processing.process_one import process_image
...
async def process_user_images(
    ...
    image_files = [
        file for file in directory.rglob("*") if file.suffix.lower() in app_config.image_suffixes
    ]
    ...
    to_process_zip = list(zip(photos_to_process, failed_photos, strict=False)) + list(
        zip(videos_to_process, failed_videos, strict=False),
    )
