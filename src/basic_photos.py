...
def pillow_process(image_path, thumbnail_sizes, output_dir):
...
def insert_image_metadata(session, image_info):
    try:
...
def image_exists(session, filename):
    return session.query(exists().where(ImageModel.filename == filename)).scalar()


def image_exists_by_hash(session, image_hash):
...
def process_image(image_path, thumbnail_sizes, output_dir, session):
    # Compute the hash of the image
    image_hash = hash_image(image_path)

    # Check if an image with this hash already exists in the database
    if image_exists_by_hash(session, image_hash):
...
def process_images_in_directory(directory_path, thumbnail_sizes, output_dir):
    image_files = []
    ...
    def on_created(self, event):
        ...
        try:
            ...
            if not event.is_directory and event.src_path.lower().endswith(
                ...
                process_image(event.src_path, self.thumbnail_sizes, self.output_dir, session)
        finally:
            session.close()
...
def process_all():
    ...
    process_images_in_directory(constants.PHOTOS_DIR, constants.THUMBNAIL_SIZES, constants.THUMBNAILS_DIR)
